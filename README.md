...
3.0.14 修复手机更新大安装包时的内存溢出闪退：更新下载改为真正流式写入缓存，补齐中断清理与响应大小检查，不更改聊天、模型或手表逻辑。

本次增加手机与 OPPO Watch 3 蓝牙陪伴、独立人设长期记忆，并修复悬浮窗收起、头像串用和主动消息阻塞。连接、隐私与真实限制见 [同一个角色的连续经历](docs/WATCH_ARCHITECTURE.md)。不需要手表登录 Tailscale。

本次工作区与页面陪伴说明见 [3.0.10 更新与验收](docs/COMPANION_3_0_10.md)。画面共享使用 Android 系统授权，不依赖无障碍；文字感知与应用操作仍分别授权。
