...
class MainActivity : ComponentActivity() {
    ...
    private fun handleProactiveIntent(intent: Intent?) {
        ...
        if (source !in setOf("normal", "rp")) return
        ProactiveNavigation.open(
            ProactiveDestination(
                ...
                worldId = intent?.getStringExtra(ProactiveNotifications.EXTRA_WORLD).orEmpty(),
                characterId = intent?.getStringExtra(ProactiveNotifications.EXTRA_CHARACTER).orEmpty()
            )
        )
